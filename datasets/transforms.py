...
@register_module
class DropDataKey(object):
    def __init__(self, prob=0.3, key='color'):
        self.prob = prob
        self.key = key

    def __call__(self, data_dict):
        key = self.key
        if key in data_dict.keys() and random.random() < self.prob:
            data_dict[key] = np.zeros_like(data_dict[key])
        return data_dict


@register_module
class GridSampleProbabilistic(object):
    """
    This module will randomly assign a grid size to the input point clouds to make the network robust to different grid sizes. 
    """
    def __init__(
        self,
        grid_sizes=[0.05],
    ...
    def __call__(self, data_dict):
        # Choose a random grid size from the list of grid sizes
        grid_size = np.random.choice(self.grid_sizes, p=self.probs)
...
class SphereCrop(object):
    ...
    def __call__(self, data_dict):
        ...
        # mode is "random" or "center"
            ...
            if "panoptic" in data_dict.keys():
                data_dict["panoptic"] = data_dict["panoptic"][idx_crop]
            if "instance" in data_dict.keys():
            ...
            if "masks" in data_dict.keys():
                data_dict["masks"] = data_dict["masks"][idx_crop]
            if "mask" in data_dict.keys():
                data_dict["mask"] = data_dict["mask"][idx_crop]
        return data_dict
...
class Compose(object):
    def __init__(self, cfg=None):
        ...
        for t_cfg in self.cfg:
            name = t_cfg["type"]
            t_cfg.pop("type")
            transform = TRANSFORMS[name](**t_cfg)
