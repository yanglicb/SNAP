...
class DefaultDataset_new(Dataset):
    def __init__(
        self,
        split="train",
        data_root="data/dataset",
        ...
        super(DefaultDataset_new, self).__init__()
        self.data_root = data_root
        self.split = split
        self.transform1 = Compose(transform[:-2])
        self.transform2 = Compose(transform[-2:])
        self.loop = (
            loop if not test_mode else 1
        )  # force make loop = 1 while in test mode
        self.test_mode = test_mode
    ...
    def prepare_train_data(self, idx):
        # load data
        data_dict = self.get_data(idx)
        # print("afte getting data: ", data_dict.keys())
        data_dict = self.transform1(data_dict)
        # print("After first transform: ", data_dict.keys())
        data_dict = self.process_data(data_dict)
        # print("after processing data: ", data_dict.keys())
        data_dict = self.transform2(data_dict)
        # print("After transform 2: ", data_dict.keys())
        return data_dict
