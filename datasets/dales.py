...
class DalesDataset(DefaultDataset_new):
    def __init__(
        self,
        split="train",
        data_root="",
    ...
    def get_data_list(self):
        if self.split == "val":
            filenames = glob(osp.join(self.data_root, "dales_ply_processed_50m", "test", "*.npy"))
        else:
            filenames = glob(osp.join(self.data_root, "dales_ply_processed_50m", self.split, '*.npy'))
    ...
    def get_data(self, idx):
        ...
        coord = pointcloud[:, :3]
        segment = pointcloud[:, 3]
        instance = pointcloud[:, 4]
        
        data_dict = dict(coord=coord, instance=instance, segment=segment)
        return data_dict
    ...
    def process_data(self, data_dict):
        ...
        data_dict = dict(coord=coord, grid_coord=grid_coord, point=prompt_points, 
